...
document.querySelectorAll('.menu-item').forEach(item=>{
 item.addEventListener('click', ()=>{
   ...
   const eng=item.querySelector('.english-name').innerText;
   const sizeBox=item.querySelector('.product-sizes');
   const ps=parseFloat(sizeBox.dataset.small);
   const pm=parseFloat(sizeBox.dataset.medium);
   const pl=parseFloat(sizeBox.dataset.large);
   document.getElementById('popupImg').src=img;
   document.getElementById('popupTitle').innerText=title;
   document.getElementById('popupDesc').innerText=desc;
   document.getElementById('popupEnglish').innerText=eng;
   document.getElementById('popupDynamicPrice').innerText=pm.toFixed(3)+" BD";
   document.getElementById('productPopup').style.display='flex';
   document.querySelectorAll('input[name="size"]').forEach(btn=>{
     btn.onclick=()=>{
       let np = btn.value==="small"?ps : btn.value==="large"?pl:pm;
       document.getElementById('popupDynamicPrice').innerText=np.toFixed(3)+" BD";
     }
   });
 });
});
...
document.getElementById('productPopup').addEventListener('click',(e)=>{
 if(e.target.id==='productPopup') document.getElementById('productPopup').style.display='none';
});
